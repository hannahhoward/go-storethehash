...
func assertCommonPrefixTrimmed(t *testing.T, key1 []byte, key2 []byte, expectedKeyLength int) {
	...
	require.NoError(t, err)
	err = index.Put(key1, store.KeyedBlock{Block: store.Block{Offset: 0, Size: 1}, KeySize: 1})
	require.NoError(t, err)
	_, err = index.Flush()
	require.NoError(t, err)
	err = index.Sync()
	require.NoError(t, err)
	err = index.Put(key2, store.KeyedBlock{Block: store.Block{Offset: 1, Size: 1}, KeySize: 1})
	require.NoError(t, err)
	_, err = index.Flush()
	require.NoError(t, err)
	err = index.Sync()
...
func TestIndexPutPrevAndNextKeyCommonPrefix(t *testing.T) {
	...
	err = index.Put(key2, store.KeyedBlock{Block: store.Block{Offset: 1, Size: 1}, KeySize: 1})
	require.NoError(t, err)
	err = index.Put(key3, store.KeyedBlock{Block: store.Block{Offset: 1, Size: 1}, KeySize: 1})
	require.NoError(t, err)
	_, err = index.Flush()
	require.NoError(t, err)
	err = index.Sync()
...
func TestIndexGet(t *testing.T) {
	...
	err = index.Put(key2, store.KeyedBlock{Block: store.Block{Offset: 1, Size: 1}, KeySize: 1})
	require.NoError(t, err)
	err = index.Put(key3, store.KeyedBlock{Block: store.Block{Offset: 2, Size: 1}, KeySize: 1})
	require.NoError(t, err)

	firstKeyBlock, found, err := index.Get(key1)
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, firstKeyBlock, store.KeyedBlock{Block: store.Block{Offset: 0, Size: 1}, KeySize: 1})

	secondKeyBlock, found, err := index.Get(key2)
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, secondKeyBlock, store.KeyedBlock{Block: store.Block{Offset: 1, Size: 1}, KeySize: 1})

	thirdKeyBlock, found, err := index.Get(key3)
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, thirdKeyBlock, store.KeyedBlock{Block: store.Block{Offset: 2, Size: 1}, KeySize: 1})
